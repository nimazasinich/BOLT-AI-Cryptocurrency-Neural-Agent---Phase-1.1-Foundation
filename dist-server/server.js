...
app.get('/api/data-pipeline/status', (req, res) => {
    try {
        ...
        logger.error('Failed to get data pipeline status', {}, error);
        res.status(500).json({
...
app.post('/api/data-pipeline/emergency-mode', async (req, res) => {
    try {
        ...
        logger.error('Failed to toggle emergency mode', {}, error);
        res.status(500).json({
...
app.post('/api/data-pipeline/add-symbol', async (req, res) => {
    try {
        ...
        logger.error('Failed to add watched symbol', { symbol: req.body.symbol }, error);
        res.status(500).json({
...
app.post('/api/binance/toggle-testnet', (req, res) => {
    try {
        ...
        logger.error('Failed to toggle testnet', {}, error);
        res.status(500).json({
...
app.get('/api/ai/test-initialization', (req, res) => {
    try {
        ...
        const weights = XavierInitializer.initializeLayer(layerType, Number(inputSize), Number(outputSize));
        res.json({
            success: true,
        ...
        logger.error('Failed to test initialization', {}, error);
        res.status(500).json({
...
app.get('/api/ai/test-activations', (req, res) => {
    try {
        ...
        logger.error('Failed to test activations', {}, error);
        res.status(500).json({
...
app.post('/api/ai/create-network', (req, res) => {
    try {
        ...
        switch (architecture) {
            case 'lstm':
                networkConfig = NetworkArchitectures.createLSTMNetwork(inputFeatures, params.sequenceLength || 60, params.hiddenSizes || [128, 64], outputSize);
                break;
            case 'cnn':
                networkConfig = NetworkArchitectures.createCNNNetwork(params.inputHeight || 32, params.inputWidth || 32, params.channels || 1, outputSize);
                break;
            case 'attention':
                networkConfig = NetworkArchitectures.createAttentionNetwork(inputFeatures, params.attentionHeads || 8, params.hiddenSize || 256, outputSize);
                break;
            case 'hybrid':
                networkConfig = NetworkArchitectures.createHybridNetwork(inputFeatures, params.sequenceLength || 60, outputSize);
                break;
            default:
...
app.post('/api/ai/train-epoch', async (req, res) => {
    try {
        ...
        logger.error('Failed to train epoch', {}, error);
        res.status(500).json({
...
app.post('/api/ai/predict', async (req, res) => {
    try {
        ...
        logger.error('Failed to generate prediction', { symbol: req.body.symbol }, error);
        res.status(500).json({
...
app.post('/api/ai/extract-features', async (req, res) => {
    try {
        ...
        logger.error('Failed to extract features', { symbol: req.body.symbol }, error);
        res.status(500).json({
...
app.post('/api/ai/backtest', async (req, res) => {
    try {
        ...
        const marketData = await database.getMarketData(symbol.toUpperCase(), '1h', 10000, startDate, endDate);
        const config = {
            startDate,
            endDate,
            initialCapital,
...
app.get('/api/providers/:category', async (req, res) => {
    try {
        ...
        const providers = await providerRegistry.getProvidersByCategory(category);
        ...
        logger.error('Failed to fetch providers by category', { category: req.params.category }, error);
        res.status(500).json({
...
app.get('/api/providers/search/:query', async (req, res) => {
    try {
        ...
        logger.error('Failed to search providers', { query: req.params.query }, error);
        res.status(500).json({
...
app.get('/api/providers/stats', async (req, res) => {
    try {
        ...
        logger.error('Failed to fetch provider statistics', {}, error);
        res.status(500).json({
...
app.delete('/api/alerts/:id', (req, res) => {
    try {
        ...
        logger.error('Failed to delete alert', { id: req.params.id }, error);
        res.status(500).json({
...
app.get('/api/market-data/:symbol', async (req, res) => {
    try {
        ...
        const cachedData = await database.getMarketData(symbol.toUpperCase(), interval, Number(limit));
        ...
        const marketData = await binanceService.getKlines(symbol, interval, Number(limit));
        ...
        if (dataType === 'klines') {
            ...
            binanceWs.on('message', (message) => {
                try {
                    ...
                    logger.error('Failed to process Binance WebSocket message', {}, error);
                }
            });
        }
    }
    catch (error) {
        logger.error('Failed to handle subscription', data, error);
        ws.send(JSON.stringify({
            type: 'error',
