...
export class EmergencyDataFallbackService {
    ...
    async getEmergencyMarketData(symbol) {
        ...
        for (const source of this.fallbackSources) {
            try {
                ...
                this.logger.error(`Failed to fetch from ${source}`, { symbol }, error);
            }
        }
    ...
    async fetchFromCoinGecko(symbol) {
        try {
            ...
            if (!coinId)
                return null;
            const response = await axios.get(`https://api.coingecko.com/api/v3/simple/price?ids=${coinId}&vs_currencies=usd&include_24hr_change=true&include_24hr_vol=true`, { timeout: 10000 });
            ...
            this.logger.error('CoinGecko fallback failed', { symbol }, error);
            return null;
        }
    }
    async fetchFromCoinMarketCap(symbol) {
        try {
            ...
            if (!coinSlug)
                return null;
            const response = await axios.get(`https://coinmarketcap.com/currencies/${coinSlug}/`, {
                timeout: 10000,
                headers: {
            ...
            this.logger.error('CoinMarketCap fallback failed', { symbol }, error);
            return null;
        }
    }
    async fetchFromCryptoCompare(symbol) {
        try {
            const baseCurrency = symbol.replace('USDT', '').replace('USD', '');
            const response = await axios.get(`https://min-api.cryptocompare.com/data/price?fsym=${baseCurrency}&tsyms=USD`, { timeout: 10000 });
            ...
            this.logger.error('CryptoCompare fallback failed', { symbol }, error);
            return null;
        }
    }
