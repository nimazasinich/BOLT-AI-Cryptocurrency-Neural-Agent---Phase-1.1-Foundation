...
export class NetworkArchitectures {
     ...
     */
    createLSTMNetwork(inputFeatures, sequenceLength, hiddenSizes, outputSize) {
        const layers = [];
        // LSTM layers
     ...
     */
    createCNNNetwork(inputHeight, inputWidth, channels, outputSize) {
        const layers = [];
     ...
     */
    createAttentionNetwork(inputFeatures, attentionHeads, hiddenSize, outputSize) {
        const layers = [];
     ...
     */
    createHybridNetwork(inputFeatures, sequenceLength, outputSize) {
        const layers = [];
    ...
    initializeNetwork(config) {
        ...
        for (let i = 0; i < config.layers.length; i++) {
            ...
            const layerWeights = this.initializer.initializeLayer(layer.type, layer.inputSize, layer.outputSize, this.getGainForActivation(layer.activation));
    ...
    getGainForActivation(activation) {
        switch (activation) {
            case 'relu':
            case 'leakyRelu':
