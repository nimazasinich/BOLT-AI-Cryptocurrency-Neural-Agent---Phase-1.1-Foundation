...
export class BullBearAgent {
    ...
    async initialize() {
        try {
            ...
            this.logger.error('Failed to initialize Bull/Bear agent', {}, error);
            throw error;
        }
    }
    updateGoalConfig(config) {
        this.goalConfig = { ...this.goalConfig, ...config };
        this.logger.info('Goal configuration updated', this.goalConfig);
    }
    async predict(marketData, currentGoal) {
    ...
    async performMCDropout(features) {
        const predictions = [];
    ...
    calculateMeanProbabilities(predictions) {
        const mean = predictions.reduce((acc, pred) => ({
    ...
    generateReasoning(features, probabilities, action) {
        const reasoning = [];
    ...
    async trainOnMarketData(marketData, labels) {
        if (!this.isInitialized) {
            throw new Error('Bull/Bear agent not initialized');
        }
        try {
            ...
            this.logger.error('Failed to train on market data', {}, error);
            throw error;
        }
    }
    getModelStatistics() {
