...
export class GradientClipper {
     ...
     */
    clipGradientsByGlobalNorm(gradients) {
        ...
        if (!isFinite(globalNorm)) {
            if (this.config.errorIfNonFinite) {
                throw new Error(`Non-finite gradient norm detected: ${globalNorm}`);
            }
            else {
        ...
        // Apply clipping
        const clippedGradients = gradients.map(layer => layer.map(row => row.map(grad => grad * scaleFactor)));
        if (wasClipped) {
    ...
    calculateGlobalNorm(gradients) {
        ...
        switch (this.config.normType) {
            ...
            case 'inf':
                ...
                return maxAbs;
            default:
                throw new Error(`Unsupported norm type: ${this.config.normType}`);
        }
    }
    /**
